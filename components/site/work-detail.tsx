...
export function WorkDetail({
  ...
  const defaultLabel = work.type === "featured"
    ? `CASE ${num} · AI 통합 풀스택`
    : work.type === "building"
      ? `IN PROGRESS · ${num}`
      : `CASE ${num}`
  ...
  return (
    <article className="grid lg:grid-cols-2 gap-15 py-20 border-b border-border last:border-b-0">
      {/* 비주얼 */}
      <div className={`relative aspect-[4/5] rounded-md overflow-hidden border border-border bg-zinc-900 ${imageRight ? "lg:order-2" : ""}`}>
        {thumb ? (
          <Image
            src={thumb.url}
            alt={thumb.alt || title}
          ...
          <div className="absolute inset-0 grid place-items-center text-zinc-600 font-mono text-sm tracking-wider">
            SCREENSHOT
          </div>
        )}
        {work.type === "building" && (
          <div className="absolute top-4 left-4 font-mono text-[10px] tracking-widest bg-primary text-primary-foreground px-2.5 py-1 rounded flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
            진행 중
          </div>
        )}
        {work.year && (
          <div className="absolute bottom-4 right-4 font-mono text-[10px] tracking-widest bg-background/85 backdrop-blur text-foreground px-2.5 py-1 rounded">
            {work.year}
          </div>
        )}
      </div>
